"""
Prop Firm Paper Trading Wrapper Script.

Forked from run_paper_trading.py for prop firm accounts.
Runs runners.prop_firm.run_live instead of runners.run_live.
Logs to separate directory: logs/prop_paper_trading/
...
PROP_VERSION = f"{STRATEGY_VERSION}-PROP"

# Configuration
MAX_RESTARTS_PER_DAY = 5
LOG_DIR = Path(__file__).parent / "logs" / "prop_paper_trading"
...
def get_log_file():
    ...
    return LOG_DIR / f"prop_paper_trading_{today}.log"
...
def run_health_check():
    ...
    core_imports = [
        ...
        ("runners.prop_firm.run_live", "LiveTrader"),
    ]
...
def run_paper_trading(use_webhook=False, symbols=None):
    """Run the prop firm paper trading script and capture output."""
    sym_list = symbols or ["ES"]
    cmd = [
        sys.executable, "-m", "runners.prop_firm.run_live",
        "--paper",
        "--symbols", *sym_list,
    ]
    if use_webhook:
    ...
    with open(log_file, "a") as f:
        ...
        while process.poll() is None:
            ...
            if not is_market_hours():
                log("Market closed - stopping prop paper trading")
                process.terminate()
                try:
            ...
            if (now - last_wrapper_heartbeat).total_seconds() >= WRAPPER_HEARTBEAT_INTERVAL:
                ...
                log(f"[PROP WRAPPER] {now.strftime('%H:%M:%S')} | Bot running (PID: {process.pid}) | Uptime: {uptime_str}")
...
def main():
    ...
    log("=" * 60)
    log(f"Prop Firm Paper Trading Wrapper {PROP_VERSION} Started{mode_str}{sym_str}")
    log("=" * 60)
    ...
    log("")
    log("=" * 40)
    log("PRE-FLIGHT HEALTH CHECK (PROP)")
    log("=" * 40)
    ...
    while restart_count < MAX_RESTARTS_PER_DAY and is_market_hours():
        log(f"Starting prop paper trading (attempt {restart_count + 1}/{MAX_RESTARTS_PER_DAY})")
        ...
        try:
            ...
            if exit_code == 0:
                log("Prop paper trading exited normally")
                break
            else:
                log(f"Prop paper trading crashed with exit code {exit_code}")
                restart_count += 1
    ...
    log("Prop Firm Paper Trading Wrapper Finished")
    log("=" * 60)
