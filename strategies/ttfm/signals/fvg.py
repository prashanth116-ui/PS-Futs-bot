...
from strategies.ttfm.types import POI
...
def swings_as_pois(swings: list, bars: list[Bar]) -> list[POI]:
    ...
    Uses a small zone around the swing price (high/low of the swing bar).
    """
    from strategies.ttfm.types import SwingPoint
    ...
    for s in swings:
        ...
        if s.swing_type == "HIGH":
            pois.append(POI(
                poi_type="SWING",
                high=bar.high,
                low=max(bar.open, bar.close),  # Body top
                direction="BEARISH",
            ...
            pois.append(POI(
                poi_type="SWING",
                high=min(bar.open, bar.close),  # Body bottom
                low=bar.low,
                direction="BULLISH",
                bar_index=s.bar_index,
                timestamp=s.timestamp,
