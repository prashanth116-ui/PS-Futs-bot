"""Plot V10.13 strategy for a specific date."""
import sys
sys.path.insert(0, '.')
...
def plot_v10_date(symbol, target_date, contracts=3):
    ...
    ax.set_title(f'{symbol} 3-Minute | {target_date} | V10.13 Quad Entry (Hybrid Exit)\n'
    ...
    # Summary box
    summary_lines = ['V10.13 HYBRID EXIT', f'Symbol: {symbol}', f'BOS: {bos_status}', f'Trades: {len(all_results)}', f'  Creation: {creation_count}', f'  Overnight: {overnight_count}', f'  Intraday: {intraday_count}', f'  BOS: {bos_count}', '-' * 20]
    for result in all_results:
        entry_type = result['entry_type']
        direction = result['direction']
    ...
    plt.tight_layout()
    filename = f'backtest_{symbol}_V10.13_{target_date}.png'
