...
Usage:
    python -m runners.plot_ttfm ES 3
    python -m runners.plot_ttfm NQ 3
    python -m runners.plot_ttfm ES 3 --date=2026-02-24
"""
import sys
sys.path.insert(0, '.')
...
from runners.run_ttfm import _build_daily_bars
...
def plot_ttfm(symbol='ES', contracts=3, t1_r=2, trail_r=4, target_date=None):
    ...
    # Compute signals for overlay (match runner: LTF = 15m)
    ...
    def _15m_to_3m(idx_15m):
        """Map a 15m bar index to the nearest 3m chart x-position."""
    ...
    # ── Overlay swing points (from 15m, mapped to 3m chart) ───────────────
    ...
    # ── Overlay candle labels (C2, C3, C4 from 15m) ──────────────────────
    ...
    # ── Overlay CISD zones (from 15m, mapped to 3m chart) ───────────────────
...
if __name__ == '__main__':
    # Parse flags and positional args separately
    positional = []
    t1_r = 2
    trail_r = 4
    target_date = None
