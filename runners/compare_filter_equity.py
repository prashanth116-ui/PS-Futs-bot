"""
Compare Baseline vs Hybrid filter modes for SPY and QQQ.
"""
import sys
sys.path.insert(0, '.')
...
def backtest_equity_filter_modes(symbol, days=30, risk_per_trade=500):
    tick_size = 0.01
    min_fvg_pts = 0.20 if symbol == 'SPY' else 0.40
    min_risk_pts = 0.30 if symbol == 'SPY' else 0.50
    ...
    baseline = {'trades': 0, 'wins': 0, 'pnl': 0, 'max_dd': 0, 'peak_pnl': 0, 'losing_days': 0, 'daily_pnl': {}}
    hybrid = {'trades': 0, 'wins': 0, 'pnl': 0, 'max_dd': 0, 'peak_pnl': 0, 'losing_days': 0, 'daily_pnl': {}}

    for target_date in dates:
        day_bars = [b for b in all_bars if b.timestamp.date() == target_date]
        session_bars = [b for b in day_bars if dt_time(9,30) <= b.timestamp.time() <= dt_time(16,0)]

        if len(session_bars) < 50:
            continue

        body_sizes = [abs(b.close - b.open) for b in session_bars[:50]]
        avg_body = sum(body_sizes) / len(body_sizes) if body_sizes else 0.10
        ...
        for fvg in day_fvgs:
            ...
            mandatory_2_size = fvg_size >= min_fvg_pts
            ...
            stop_price = fvg.low - 0.05 if is_long else fvg.high + 0.05
    ...
    baseline['losing_days'] = sum(1 for p in baseline['daily_pnl'].values() if p < 0)
    hybrid['losing_days'] = sum(1 for p in hybrid['daily_pnl'].values() if p < 0)
...
if __name__ == '__main__':
    ...
    print('='*90)
    print(f'{days}-DAY BACKTEST: BASELINE vs HYBRID - EQUITIES (SPY/QQQ)')
    print('='*90)
    print()
    print('MANDATORY: DI Direction + FVG Size >= min')
    print('OPTIONAL (2/3): Displacement >= 1x | ADX >= 11 | EMA Trend')
    print()

    results = {}
    for symbol in ['SPY', 'QQQ']:
        baseline, hybrid = backtest_equity_filter_modes(symbol, days=days)
