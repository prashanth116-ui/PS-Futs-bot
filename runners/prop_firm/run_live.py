"""
V10.16-PROP Live Trading Runner - Prop Firm Futures Only

Forked from runners/run_live.py for prop firm accounts.
Futures only (ES, NQ, MES, MNQ) — no equity scanning.
Telegram messages prefixed with [PROP] to distinguish from personal.

Usage:
    python -m runners.prop_firm.run_live --paper --symbols ES MES
    python -m runners.prop_firm.run_live --paper --webhook --symbols ES MES
"""
import sys
sys.path.insert(0, '.')
...
# Prop firm local imports
from runners.prop_firm.run_v10_dual_entry import run_session_v10, is_swing_high, is_swing_low
from runners.prop_firm.risk_manager import RiskManager, create_default_risk_manager
from runners.prop_firm.symbol_defaults import get_live_futures_config, get_session_v10_kwargs, get_symbol_config, FUTURES_DEFAULTS
...
PROP_STRATEGY_VERSION = f"{STRATEGY_VERSION}-PROP"
...
class LiveTrader:
    """
    V10.16-PROP Live Trading System - Prop Firm Futures Only

    Forked from runners/run_live.py. Futures only — no equity scanning.
    Telegram messages prefixed with [PROP].

    V10.16: Global consecutive loss stop (ES/MES: 2 consec losses → stop for day)
    """

    # Futures symbol configurations — from prop firm symbol_defaults.py
    FUTURES_SYMBOLS = {sym: get_live_futures_config(sym) for sym in FUTURES_DEFAULTS}

    # No equity symbols in prop firm fork
    EQUITY_SYMBOLS = {}
    ...
    def __init__(
        ...
        """
        Initialize prop firm live trader (futures only).

        Args:
            client: Tradovate API client (None for paper mode)
            risk_manager: Risk manager instance (prop firm defaults)
            paper_mode: If True, only log signals without executing
            symbols: List of futures symbols to trade (default: ['ES', 'MES'])
            executor: Executor backend (WebhookExecutor). None = no broker execution.
        """
        self.client = client
    ...
    def start(self):
        ...
        self.running = True
        print("=" * 70)
        print(f"{PROP_STRATEGY_VERSION} LIVE TRADER - Prop Firm Futures")
        print("=" * 70)
        ...
        print("Futures hours: 4:00-16:00 ET")
        print("=" * 70)
        ...
        notify_status(f"[PROP] {PROP_STRATEGY_VERSION} {mode} Trading started\nSymbols: {', '.join(self.symbols)}")
    ...
    def _scan_equity_symbol(self, symbol: str):
        ...
        # Prop firm fork: equity scanning disabled
        log(f"  {symbol}: equity scanning not supported in prop firm fork")
        return
...
def main():
    """Main entry point for prop firm live trading."""
    parser = argparse.ArgumentParser(description=f'{PROP_STRATEGY_VERSION} Live Trading - Prop Firm Futures')
    ...
    parser.add_argument('--symbols', nargs='+', default=['ES', 'MES'],
                       help='Futures symbols to trade (ES, NQ, MES, MNQ)')
    parser.add_argument('--webhook', action='store_true',
                       help='Enable PickMyTrade webhook execution')
    parser.add_argument('--strategy-group', default='ict_v10_prop',
                       help='PickMyTrade strategy group (default: ict_v10_prop)')
    parser.add_argument('--webhook-config', default='config/pickmytrade_accounts_prop.json',
                       help='Path to PickMyTrade config (default: config/pickmytrade_accounts_prop.json)')
    args = parser.parse_args()

    # Validate symbols (futures only)
    valid_futures = ['ES', 'NQ', 'MES', 'MNQ']

    for sym in args.symbols:
        if sym not in valid_futures:
            print(f"Invalid symbol: {sym}")
            print(f"Valid symbols: {', '.join(valid_futures)}")
            return
    ...
    print(f"[PROP] Starting {PROP_STRATEGY_VERSION} Live Trader...")
    print(f"Environment: {environment.upper()}")
    print(f"Paper Mode: {paper_mode}")
    print(f"Futures: {', '.join(args.symbols)} (2-tick buffer)")
