...
def _manage_trades(
    ...
    """Check stops, T1 exits, and trail updates for all active trades."""
    to_remove = []
    ...
    for trade in active_trades:
        ...
        if trade['t1_hit'] and not trade['trail_active'] and remaining > 0:
        ...
        if trade['trail_active'] and remaining > 0:
...
def run_session_ttfm_native(
    ...
    for i in range(2, len(bars_15m_session)):
        ...
        if lbl is None or lbl.label != entry_on:
            continue
        if not in_session(bar.timestamp):
            continue
